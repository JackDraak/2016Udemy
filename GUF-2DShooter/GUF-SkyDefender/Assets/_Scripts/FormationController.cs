...
public class FormationController : MonoBehaviour {
	...
	void Start () {
		gameManager = GameObject.FindObjectOfType<GameManager>();
			if (!gameManager) Debug.LogError ("GAME_MANAGER_FAIL_Start");
	...
	void FixedUpdate () {
		...
		if (gameManager.GetScore() > 50000f) {
			Despawner();
			gameManager.WinBattle();
		}

		// win after X waves?
		if (gameManager.GetWaveNumber() == finalWave && FormationIsEmpty()) {
			Despawner();
			gameManager.WinBattle();
		}
		...
		if (FormationIsFull()) { respawn = false; gameManager.HideWave(); }
		if (FormationIsEmpty() && !respawn && !afterMatch) { TriggerRespawn(); }
		if (gameManager.GetEnemies() == 0 && !respawn && gameStarted && !afterMatch) { TriggerRespawn(); }
	}
	...
	void Respawn () {
		gameManager.ShowWave();
		Transform freePos = NextFreePosition();
		if (freePos) FillPosition(freePos);
		if (NextFreePosition()) Invoke("Respawn", spawnDelay);
		else if (FormationIsFull() && gameManager.GetWaveNumber() < finalWave) gameManager.IncrementWaveNumber();
	}
